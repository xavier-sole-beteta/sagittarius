...
// Aquest és el codi per ponderar els atributs
        	if(!m_DontNormalize) {
        		if(Configuracio.opcio == Configuracio.PESOS_KMEANS) {
        			//System.out.println("PESOS K-MEANS (GrupsDeTreball)");
                	return (norm(val1, index) - norm(val2, index))*ConfiguracioRellevanciaKMeans.pesosAtributs[index];
        		}
        		if(Configuracio.opcio == Configuracio.PESOS_XMEANS) {
        			//System.out.println("PESOS X-MEANS (Perfils)");
                	return (norm(val1, index) - norm(val2, index))*ConfiguracioRellevanciaXMeans.pesosAtributs[index];
        		}
        		System.out.println("[ECOA] Funció EUCLIDIANA sense pesos");
            	return (norm(val1, index) - norm(val2, index));
        	}else{
        		return (val1 - val2);
        	}

// Aquest és el codi que hi havia
/*          return (!m_DontNormalize) ? 
